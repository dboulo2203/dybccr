...
import { displayActionSendRegistration, checkAlreadyRegisteredActivities, getSelectedLines } from './actionSendRegistration/actionSendRegistration.js';
...
function attachPaymentListeners() {
  const panels = ['paymentHelloAsso', 'paymentCheque', 'paymentAutre'];
...
async function validateRegistration() {
  ...
  let cgvValid = cgvCheck.checked && errors.length === 0;

  // Vérification que l'utilisateur n'est pas déjà inscrit aux mêmes activités, pour la même année
  if (cgvValid) {
    const alreadyRegistered = await checkAlreadyRegisteredActivities(email, getSelectedLines());
    if (alreadyRegistered.length > 0) {
      cgvCheck.checked = false;
      cgvError.textContent = `Vous êtes déjà inscrit(e) pour cette année à : ${alreadyRegistered.join(', ')}.`;
      cgvValid = false;
    }
  }
  ...
  const outputStr = `
    ...
    <div class="p-3">
      ...
      <div class="accordion" id="paymentAccordion">
        ...
        <div class="accordion-item">
          <h2 class="accordion-header">
            <button class="accordion-button collapsed" disabled type="button" data-bs-toggle="collapse" data-bs-target="#paymentAutre">
              <i class="bi bi-arrow-left-right me-2"></i>Je règle par Dispositif Sortir, en espèces, en chèques vacances, en plusieurs fois
            </button>
          </h2>
          <div id="paymentAutre" class="accordion-collapse collapse" data-bs-parent="#paymentAccordion">
            <div class="accordion-body">
              <p class="text-muted">Lorsque vous cliquerez sur 'Envoyer' votre inscription sera transmise au cercle celtique. Vous devrez ensuite passer au cercle pour régler votre inscription.
            .</p>
            <div id="messageSectionSend"></div>
