...
import { getInvoice } from '../../shared/appWSServices/dolibarrInvoicesServices.js'
...
export async function displaySearchContent(htlmPartId) {
  ...
  async function runSearch() {
    ...
    if (/^\d+$/.test(searchString.trim())) {
      const invoice = await getInvoice(searchString.trim());
      globalThis.location.href = `${getAppPath()}/views/person?paramid=` + invoice.socid;
      return;
    }
