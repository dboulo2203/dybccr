...
if (!$res)
	...
	// require_once DOL_DOCUMENT_ROOT . '/core/class/html.formfile.class.php';
	 ...
	 require_once DOL_DOCUMENT_ROOT . '/custom/dybccr/lib/dybccr.lib.php';
	...
	print load_fiche_titre($langs->trans("Dybccr Clôture de caisse pour la période"), '', 'favicon.png@dklcompta');
	...
	// if (!$user->rights->dybccr->myobject->read)
...
// *** Liste des factures
    ...
    print "<br><hr><h2>Factures impliquées dans la clôture de caisse </h2> ";



    $sql = "select u.login as user, f.`type` as ftype, f.date_closing, f.rowid, t.nom as societe, f.ref, f.total_ttc from llx_facture as f

left join llx_societe as t on f.fk_soc=t.rowid
...
where f.paye=1 and f.date_closing>='" . $firstdate->format('Y-m-d H:i:s') . "' and f.date_closing<='" . $lastdate->format('Y-m-d H:i:s') . "'";
    ...
    if ($resql) {
       ...
       print '<th colspan="3">'.$langs->trans("Nombre de factures").($num?'<span class="badge marginleftonlyshort">'.$num.'</span>':'').'</th></tr>';

        print "<tr>";

		print "<th>User</th>";

        print "<th>type facture</th>";
        ...
        for ($i = 1; $i <= $num; ++$i) {
            ...
            print "<td>" . ($obj->ftype == 0 ? "Facture" : ($obj->ftype == 3 ? "Acompte" : "Autre")) . "</td>"; // '$obj->ftype

            print "<td><a href='" . $dolibarr_main_url_root . "/compta/facture/card.php?facid=" . $obj->rowid . "'>" . $obj->ref . "</a></td>";

            print "<td>" . $obj->date_closing . "</td>";

            print "<td>" . $obj->societe . "</td>";



            print "<td class=\"nowrap right\">" . sprintf("%01.2f", $obj->total_ttc) . "</td>";
