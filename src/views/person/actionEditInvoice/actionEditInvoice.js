 ...
 * @param {object} invoice  - Dolibarr invoice object (with lines and array_options)
...
export async function displayActionEditInvoice(invoice, customer, onSaveCallback) {
        ...
        <div class="modal-content">
          <div class="modal-header">
            <span class="modal-title fs-5">
              <i class="bi bi-receipt me-2"></i>Modifier facture ${invoice['ref']} — ${customer['name']}
            </span>
          ...
          <div class="modal-body">
            ...
            <div class="mb-3 col-6">
              ...
              <select class="form-select" id="${modalId}-culturalseason" required>
                <option value="">—</option>
                ${getSelectFromDatabaseList('yearexercice', 'rowid', 'label', currentSeason)}
              </select>
            </div>
            <div class="mb-3">
            ...
            <div class="border-top mt-3 pt-2">
              ...
              <div id="${modalId}-existing-payments" class="mt-2"></div>
  ...
  // await loadProducts();
  const [products, existingPayments] = await Promise.all([
    Promise.resolve(getAllActiveProducts().map((p) => ({ ...p, display: ` ${p.label}` }))),
    getInvoicePayments(invoice.id).catch(() => []),
  ]);
  ...
  return new Promise((resolve) => {
    ...
    // Force la valeur du select après insertion dans le DOM (évite les problèmes de type string/number)
    if (currentSeason) {
      const seasonSelect = document.getElementById(`${modalId}-culturalseason`);
      if (seasonSelect) seasonSelect.value = String(currentSeason);
    }

    // Pre-fill lines from existing invoice
    ...
    const cleanup = (result) => {
      modal.hide();
      modalElement.addEventListener('hidden.bs.modal', () => container.rproduitsemove(), { once: true });
      resolve(result);
    };

    const computeInvoiceTotal = () => lines.reduce((s, l) => s + l.qty * l.subprice, 0);
    const computeNewPaid = () => payments.reduce((s, p) => s + p.amount, 0);
    const computeRemaining = () => computeInvoiceTotal() - existingPaidTotal - computeNewPaid();
    ...
    const renderPayments = () => {
      const allTypes = getPaymentTypes() || [];
      const paymentsContainer = document.getElementById(`${modalId}-payments`);
      if (payments.length === 0) {
        paymentsContainer.innerHTML = '<p class="text-muted small">Aucun paiement.</p>';
        return;
      }
    ...
    document.getElementById(`${modalId}-add-payment`).addEventListener('click', () => {
      ...
      const remaining = Math.max(computeRemaining(), 0);
    ...
    document.getElementById(`${modalId}-save`).addEventListener('click', async () => {
      try {
        ...
        if (existingPaidTotal + computeNewPaid() > computeInvoiceTotal()) throw new Error('Le total des paiements dépasse le montant de la facture');

        if (invoice.status === '1')
          await setToDraftInvoice({ id: invoice.id });

        for (const line of invoice['lines'] || []) {
          await deleteInvoiceLine(invoice.id, line.id);
        }
        await updateInvoiceHeader(invoice.id, {
          array_options: { options_inv_culturalseason: culturalseason },
        });
        for (const line of lines) {
          await addInvoiceLine(invoice.id, line);
        }
        await validateInvoice({ id: invoice.id });

        for (const payment of payments) {
          if (payment.amount > 0) {
            await createInvoicePayment({ id: invoice.id }, payment.type, payment.amount, '', null);
          }
        }
